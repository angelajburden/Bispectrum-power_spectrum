...
void calc_power(double *dd, int NG, int NGAL, double BOX, char *name) {
  ...
  const double maxk = 1.6;//2.*pi*0.25;        // maximum k to be binned
  ...
  // fftw_execute(dp_r2c);
  //fftwnd_one_real_to_complex(dp_r2c,(fftw_real*)dd,NULL);
  ...
  for(int i=0;i<NG;i++) {
	...
	if(bin>=1 && bin<NB && fktot < Nyquist && Pgfill[bin]==0) {
	  ...
	  //Pg[bin] += ((dkr*dkr+dki*dki - (tran_weightsq + tgal_weightsq))*grid_cor*grid_cor);
          Pg[bin] += ((dkr*dkr+dki*dki)*grid_cor*grid_cor);
          if (bin ==0) Pg[bin]= 0.0;
	  Nk[bin]++;	      
	}	
      }
